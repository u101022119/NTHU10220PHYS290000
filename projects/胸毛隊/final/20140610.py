...
while run:
    ...
    if test_pe(player,enemy_red) != []:
        ...
        color_background= color_temp[0]+10,color_temp[1]-10,color_temp[2]-10
        if spd_enemy > 7:
            spd_enemy = int(spd_enemy - 3)
            color_shoot = PINK
    if life <= 0:
        run =False
    ...
    y +=20
    if color_shoot is YELLOW:
        BuildFont("BONUS!!",(x,y),scr_s)
