...
##Font initialization
def BuildFont(string,pos,scr_s):
    basicFont = pyg.font.SysFont(None,size_font)
    text = basicFont.render(string, False, WHITE)
...
while run:
    for event in pyg.event.get():
        if event.type is pyg.KEYDOWN:
            ...
            if keyname is pyg.K_ESCAPE:  
                ...
                cheat = not cheat
            elif keyname is pyg.K_z and dist_enemy < 400:
                dist_enemy += 50
            elif keyname is pyg.K_x and dist_enemy > 50:
                dist_enemy -= 50
    ...
    if gametime%100 is 0:
        EnemyCreate(enemy_blue,player,int(gametime/100)+3)
    ##Enemy Blue Moving
    ...
    if test_pe(player,enemy_red) != []:
        life -= 1
        color_temp = color_background
        color_background= color_temp[0],color_temp[1]-10,color_temp[2]-10
        if spd_enemy > 7:
            spd_enemy = int(spd_enemy - 5)
            color_shoot = PINK
    if life <= 0:
        run =False
        basicFont = pyg.font.SysFont(None,100)
